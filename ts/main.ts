...
var slice: Vector = Vector.fromAngle(randomFloat(0, Math.PI * 2), 100).unit();
    ...
    for (let i of shape1) {
        for (let j = 0; j < i.length; j++) {
            ...
            if (false && p.pos !== null) {
                ...
                ctx.beginPath();
                ctx.arc((<Vector>p.pos).x, (<Vector>p.pos).y, r, 0, Math.PI * 2);
                ctx.fill();
                ctx.closePath();
            }
...
function play() {
    ...
    for (let i of newShape) {
        ...
        const res: Shape = [];
        if (ring1.length > 0) res.push(ring1);
        if (ring2.length > 0) res.push(ring2);
        rings.push(...res);
    }
    shape1 = rings;
...
type VertexLoop = Vector[];
type Shape = VertexLoop[];
...
function createRandomPolygone(rings: number, vertex: number, size: number, RingRandomness = 2, ringSizeRng = 2, ringSizeRng2 = 0.1) {
    const shape: Shape = [];
...
function LineLineIntersection(v1: Vector, v2: Vector, v3: Vector, v4: Vector) {
    const div = ((v1.x - v2.x) * (v3.y - v4.y) - (v1.y - v2.y) * (v3.x - v4.x));
    if (div == 0) /*parallel or coincident*/ return {
        pos: null,
        u: 0,
        t: 0
    };
